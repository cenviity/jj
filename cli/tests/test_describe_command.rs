...
fn test_add_trailer_committer() -> TestResult {
    ...
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Working copy  (@) now at: qpvuntsm b85325bb (empty) Message from CLI
    Parent commit (@-)      : zzzzzzzz 00000000 (empty) (no description set)
    [EOF]
    ");
    ...
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Working copy  (@) now at: yqosqzyt 440e8be9 (empty) Signed-off-by: test.user@example.com
    Parent commit (@-)      : qpvuntsm b85325bb (empty) Message from CLI
    [EOF]
    ");
