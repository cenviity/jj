...
# Add a feature, then add a description and start a new commit.
$ jj commit -m "feat(bar): add support for bar"
# Create a bookmark so we can push it to GitHub. Note that we created the
# bookmark on the working-copy commit's *parent* because the working copy itself
# is empty.
...
In a colocated workspace, every `jj` command automatically synchronizes
