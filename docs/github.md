...
As of October 2023 it's recommended to [set up an SSH key][gh] to work with GitHub
projects. This restriction may be lifted in the future – see [issue #469][http-auth] for
more information and progress on authenticated HTTP.

## Basic workflow

The simplest way to start with Jujutsu is to create a stack of commits first.
You only need to create a bookmark when you need to push the stack to a
...
new commits. Unlike Git, Jujutsu does not do this automatically.
...
In a co-located repository, every `jj` command automatically synchronizes
...
$ jj diff
# Give the fix a description and create a new working-copy commit on top
$ jj commit -m "address pr comments"
# Update the bookmark to point to the parent commit
...
!!! warning
    ...
    We strongly suggest to `jj new` after the example below. Otherwise, all further
    edits will automatically amend the latest commit since it remains as the
    working-copy commit.
...
local bookmarks. This means that if you want to iterate on or test another
...
GitHub CLI has trouble finding the proper Git repository path in `jj` repos
...
You can make this automatic by installing [direnv](https://direnv.net) and
...
For a detailed overview of how Jujutsu handles conflicts, revisit
the [tutorial][tut].
...
merged through a pull request, while "origin" is your fork of the project.
...
push to in your [configuration file](config.md#config-files-and-toml):
