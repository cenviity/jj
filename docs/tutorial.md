...
We can see from the output above that our [working copy](glossary.md#working-copy) is a
real [commit](glossary.md#commit) with a [commit ID](glossary.md#commit-id) (`d7439b06`
in the example). When you make a change in the working copy, the
...
commit, and `bookmarks()` refers to all commits pointed to by bookmarks (similar to
...
Other commands for rewriting contents of existing commits are `jj split` and
`jj unsquash -i`. Now that you've seen how `jj squash -i` and `jj diffedit` work,
